...
if [ "$#" -ne 1 ]; then
    ...
    echo "Usage: win-ci-logs-collector.sh masterIp"
    exit 1
fi
...
for log_file in "${master_provisioning_logs[@]}"; do
    scp -i ${SSH_KEY} ${USER}@${MASTER_IP}:${log_file} "${master_logs_output}/$(basename -- ${log_file})"
    if [ ! $? -eq 0 ]
    then
...
vmname_prefixes=$(grep "windows.*VMNamePrefix\":" azuredeploy.json | awk '{ print $2 }' | tr -d ",\"")
SAVEIFS=$IFS
IFS=$'\n'
...
function collect_windows_vm_logs {
    ...
    if ! $(ssh -J ${USER}@${MASTER_IP} ${USER}@${win_hostname} "exit"); then
        return
    fi
    ...
    for log_file in "${provisioning_logs[@]}"; do
        scp -o "ProxyJump ${USER}@${MASTER_IP}" ${USER}@${win_hostname}:"c:\\${log_file}" "${win_logs_location}/$(basename -- ${log_file})"
        if [ ! $? -eq 0 ]
        then
    ...
    if ! $(ssh -J ${USER}@${MASTER_IP} ${USER}@${win_hostname} "powershell.exe -c Test-Path ${win_logs_collector_script_path}" | grep False)
    then
        echo "Downloading log collector script to Windows machine ${win_hostname}."
        if $(ssh -J ${USER}@${MASTER_IP} ${USER}@${win_hostname} "powershell.exe -c \". C:\\AzureData\\k8s\\kuberneteswindowsfunctions.ps1 ; DownloadFileOverHttp -Url ${win_logs_collector_script_url} -DestinationPath ${win_logs_collector_script_path} \""); then
    ...
    $(ssh -J ${USER}@${MASTER_IP} ${USER}@${win_hostname} "powershell.exe -c \"${win_logs_collector_script_path}\"")
    
    echo "Copying logs from Windows node ${win_hostname}"
    scp -o "ProxyJump ${USER}@${MASTER_IP}" ${USER}@${win_hostname}:"c:\\k\\Debug\\*.zip" "${win_logs_location}/debug.zip"
    if [ ! $? -eq 0 ]
    then
