...
run_remote_cmd ${VM_PUB_IP} ${SSH_KEY_FILE} 'c:/prepare_e2e_node_windows.ps1'

if [ "${JOB_TYPE}" == "presubmit" ]
then
    echo "Running a presubmit job"
    run_remote_cmd ${VM_PUB_IP} ${SSH_KEY_FILE} "c:/k8s_e2e_node_windows.ps1 -repoName ${REPO_NAME} -repoOrg ${REPO_OWNER} -pullRequestNo ${PULL_NUMBER} -pullBaseRef ${PULL_BASE_REF} ${test_packages_arg}"
    exit_code=$?
else
