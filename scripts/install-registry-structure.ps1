...
param(
    ...
    [string]$ImageRef = "registry.k8s.io/pause:3.10",
...
function Copy-FileToPath {
    ...
    Write-Verbose "Copied: $DestinationPath"
}
...
try {
    ...
    try {
        ...
        Write-Verbose "Found $($index.manifests.Count) manifest(s) in index"

        # Process the manifest list (we assume it's always a manifest list)
        Write-Info "Processing manifest list..."
        ...
        Write-Verbose "Found $($windowsManifests.Count) Windows manifest(s)"
        ...
        foreach ($winManifest in $windowsManifests) {
            ...
            Write-Verbose "Processing Windows manifest: $winDigest"
