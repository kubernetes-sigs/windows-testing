...
ENVSUBST="${CAPZ_DIR}/hack/tools/bin/envsubst"
make --directory="${CAPZ_DIR}" "${ENVSUBST##*/}"
...
main() {
    ...
    if [[ "$(az group exists --name "${GMSA_NODE_RG}")" == "false" ]]; then
        az group create --name "$GMSA_NODE_RG" --location "$AZURE_LOCATION" --tags creationTimestamp="$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
    fi
    ...
    if [[ -z $vmid ]]; then 
        ...
        GMSA_DOMAIN_ENVSUBST="${SCRIPT_ROOT}/domain.init"
        GMSA_DOMAIN_FILE="${SCRIPT_ROOT}/domain.init.tmpl"
        $ENVSUBST < "$GMSA_DOMAIN_FILE" > "$GMSA_DOMAIN_ENVSUBST"
        az vm create -l "$AZURE_LOCATION" -g "$GMSA_NODE_RG" -n "$vmname" \
            --image cncf-upstream:capi-windows:windows-2019-containerd-gen1:latest \
