...
run_e2e_test() {
    ...
    if [[ ! "$SKIP_TEST" == "true" ]]; then
        ...
        if [[ ! "${RUN_SERIAL_TESTS:-}" == "true" ]]; then
            ...
            export GINKGO_SKIP="\[LinuxOnly\]|\[Serial\]|\[Slow\]|\[Excluded:WindowsDocker\]|\[Feature:DynamicResourceAllocation\]|Networking.Granular.Checks(.*)node-pod.communication|Guestbook.application.should.create.and.stop.a.working.application|device.plugin.for.Windows|Container.Lifecycle.Hook.when.create.a.pod.with.lifecycle.hook.should.execute(.*)http.hook.properly|\[sig-api-machinery\].Garbage.collector"
            ...
            export GINKGO_SKIP="\[LinuxOnly\]|\[Excluded:WindowsDocker\]|device.plugin.for.Windows|should.be.able.to.gracefully.shutdown.pods.with.various.grace.periods|\[Alpha\]"
